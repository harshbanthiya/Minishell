...
void check_cmdinvo(t_command *actual_cmdinvo, t_command *expected_invo)
{
...
int main()
{
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo 文字列1つ");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("abc", ' '));
		CHECK(actual);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo arguments 2個");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("abc def", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo() 空文字列を含む");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo(NULL);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo リダイレクト付き");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("file", ' '));
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo 環境変数");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("echo abc", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo スペースが含まれている環境変数");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("echo abc def", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo 文字列と環境変数");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("echo hogeabc def", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo 環境変数が複数");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("echo abc defABC DEF", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo リダイレクション 環境変数");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("echo hello", ' '));
		cmd_add_outredirect(expected, "$ABC", 1, false);
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_cmd2cmdinvo 存在しない環境変数");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_cmd2cmdinvo(node->content.command);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("echo", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_pipcmds2cmdinvo 文字列1つ");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_pipcmds2cmdinvo(node->content.piped_commands);
		t_command *expected = cmd_init_cmdinvo((const char **)ft_split("abc", ' '));
		CHECK(actual);
		check_cmdinvo(actual, expected);
	...
	TEST_SECTION("cmd_ast_pipcmds2cmdinvo パイプ & リダイレクション");
	{
		...
		/* テスト */
		t_command *actual = cmd_ast_pipcmds2cmdinvo(node->content.piped_commands);
		t_command *expected_first = cmd_init_cmdinvo((const char **)ft_split("abc", ' '));
		t_command *expected_second = cmd_init_cmdinvo((const char **)ft_split("file", ' '));
